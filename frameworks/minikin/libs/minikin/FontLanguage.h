...
public:
    ...
    FontLanguage() : mScript(0ul), mLanguage(0ul), mSubScriptBits(0ul),kZGFlag(false),kMMFlag(false) {}

    // Parse from string
    FontLanguage(const char* buf, size_t length);

    bool operator==(const FontLanguage other) const {
        return !isUnsupported() && isEqualScript(other) && mLanguage == other.mLanguage && (kZGFlag == other.kZGFlag) && (kMMFlag == other.kMMFlag);
    }
